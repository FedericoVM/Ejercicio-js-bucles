...
while (confirm("¿Desea ingresar un DNI?")) {
  ...
  let dniUsuario = parseInt(prompt("Por favor ingrese el dni"));

  if (isNaN(dniUsuario)) {
    alert("El dni solo debe tener numeros. Intente nuevamente");
  } else if (dniUsuario > 0 && dniUsuario < 99999999) {
    let calcularRestoDni = Math.round(dniUsuario % 23);
    switch (calcularRestoDni) {
      case 0:
        letra = "T"
        break;
      case 1:
    ...
    alert(`El resultado indica que su DNI corresponde a la letra: ${letra}`);
  } else {
    alert("No es valido el rango. Intente nuevamente ")
  }
